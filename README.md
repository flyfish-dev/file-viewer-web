...
`<flyfish-file-viewer>` 和 Web/IIFE 默认使用 Shadow DOM 隔离组件壳与预览内容，适合 OA、低代码、微前端和后台系统这类宿主 CSS 不可控的页面。需要历史 light DOM 行为时，可传 `options: { styleIsolation: 'none' }` 或 `<flyfish-file-viewer style-isolation="none">`。

推荐通过 CSS 变量和 Shadow Parts 做受控定制：
...
稳定 parts 包括 `host`、`shell`、`toolbar`、`toolbar-group`、`toolbar-status`、`button`、`input` 和 `content`。所有标准组件默认使用 Shadow DOM；主题定制优先使用 tokens 和 `::part()`，仅旧版深层覆盖场景显式使用 `options.styleIsolation: 'none'`。完整模式和 hostile CSS 验证见官方文档: https://doc.file-viewer.app/
...
从 3.0.3 起，`dist/flyfish-file-viewer-web.amd.js` 提供轻量组件壳的 AMD API，
不自动包含格式渲染器。要直接预览文档，使用 `@file-viewer/web-full` 的 AMD 入口并完整部署其 `dist/`。
[接入示例](https://doc.file-viewer.app/zh/guide/quickstart-web#requirejsamd)。
...
| DICOM | medical-image | `.dcm`, `.dicom` | basic / experimental | 下载, 缩放(Provider) | 按需异步 |
| Digital signatures and evidence containers | cryptographic-container | `.p7m`, `.p7s`, `.p7c`, `.p7b`, `.pkcs7`, `.cms`, `.cmsc`, `.tsd`, `.tst`, `.tsq`, `.tsr`, `.asics`, `.scs`, `.asice`, `.sce`, `.ers`, `.asc`, `.sig`, `.pgp`, `.gpg`, `.jws` | structured / experimental | 下载 | 按需异步 |
...
| Adobe XD Package | asset | `.xd` | structured / experimental | 下载 | 按需异步 |
| Adobe InDesign Native | asset | `.indd`, `.indt` | structured / experimental | 下载 | 按需异步 |
| Adobe EPS and PostScript | asset | `.ps`, `.eps`（增强 `data-asset`） | experimental / experimental | 下载, 打印, 缩放 | 按需异步 |
...
| 资源 | 说明 |
| --- | --- |
| Standard 资产 | `@file-viewer/assets-standard` 只复制 PDF、DOCX、PPTX、Excel/OpenXML 与 Archive 等冻结 standard 所需 Worker、WASM、字体和 vendor 文件；事务收据记录包版本、profile hash、逐文件 SHA-256 和 owner。 |
| 专用资产 owner | CAD、Typst、iWork、3D、Data、Drawing、Hangul、WordPerfect 和旧版 PPT 各自使用独立 `@file-viewer/assets-*` 包。`@file-viewer/cli add <format> --write` 只选择该能力及其资产，不下载旁系格式。 |
